...
class BuildingInput(BaseModel):
    ...
    building_id: str = Field(
        description=(
            "Unique alphanumeric string identifying the specific building within the simulation "
            "environment."
        )
    )
    ...
    outdoor_surface_names: list[str] = Field(
        description=(
            "Collection of outdoor surface identifiers participating in the localized long-wave "
            "radiation exchange loop."
        )
    )
...
class SimulationInputs(BaseModel):
    ...
    inversion_parameters: InversionConfig = Field(
        default_factory=InversionConfig,
        description=(
            "Custom parameter convergence adjustments, bounds, and thresholds configured for the "
            "matrix solvers."
        ),
    )
