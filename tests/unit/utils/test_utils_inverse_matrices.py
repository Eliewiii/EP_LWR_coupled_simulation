...
# Replace 'your_matrix_file' with the actual name of your python script
from src.lwrepcoupling.utils.utils_inverse_matrices import InversionConfig, compute_full_inverse_via_gmres_parallel
...
def test_inversion_config_defaults():
...
def test_inversion_config_cpu_fallback():
...
def test_inversion_config_out_of_bounds_throws():
...
def test_diagonal_matrix_inversion():
...
# =====================================================================
# 3. Testing Failure Handling & Control
# =====================================================================

def test_strict_tolerance_failure_behavior():
...
def test_gmres_failure_behavior(caplog):
    """If error exceeds tol and strict_tol is True, it must raise a ValueError."""
...
def test_non_invertible_matrix_behavior(caplog):
    """Ensure a non-invertible (singular) matrix is caught by convergence checks."""
    
    # A 3x3 matrix of pure zeros is completely singular (non-invertible)
    singular_mtx = csr_matrix((3, 3), dtype=float)
    
    # Case 1: With strict_tol=False, it should log warnings but return
    ...
    # Verify the code warned the user that GMRES failed to converge
    assert any("Accuracy check failed" in record.message for record in caplog.records)

    # Case 2: With strict_tol=True, it must immediately raise a ValueError
